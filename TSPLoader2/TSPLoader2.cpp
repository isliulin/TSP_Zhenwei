...
BOOL WriteDataToFile(LPCSTR lpFileName, PCHAR lpConfigString,BYTE *pCompressBuf,DWORD nCompressSize)
{
	...
	ULONG nUnCompressSize=0;
	
	
	for (int  i = 0; i < 8; i++)
		nUnCompressSize += (UInt64)pCompressBuf[5 + i] << (i * 8);

	pBufUnCompress = new BYTE[nUnCompressSize];

	nCompressSize=nCompressSize-5-8;

	//unsigned char props[5]={0};

	int nProcess =LzmaUncompress(pBufUnCompress, (size_t*)&nUnCompressSize, pCompressBuf+5+8, (size_t*)&nCompressSize,pCompressBuf,5);
	if (0 != nProcess)
	{
