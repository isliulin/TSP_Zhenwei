...
{	
	...
	else
	{
...
#endif
		{
			...
			DWORD dwreturnue = psub->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.bottom, this, NULL);
#else
			...
			DWORD dwreturnue = ::TrackPopupMenuEx(hSubMenu, TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.bottom, m_hParentWndMenu, NULL);
#endif
			...
			if (dwreturnue)
				::PostMessage(m_hParentWndMenu, WM_COMMAND, MAKEWPARAM(dwreturnue, 0), (LPARAM)NULL);
		} // if
		else
		{
...
HICON CButtonST::CreateGrayscaleIcon(HICON hIcon)
{
	...
	BOOL		breturnue = FALSE;

	breturnue = ::GetIconInfo(hIcon, &csII);
	if (breturnue == FALSE)	return NULL;
...
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
	...
	if (hIconIn)
	{
		...
		breturnue = ::GetIconInfo(hIconIn, &ii);
		if (breturnue == FALSE)
		{
			FreeResources();
		...
		if (hIconOut)
		{
			...
			// Get icon dimension
			::ZeroMemory(&ii, sizeof(ICONINFO));
			breturnue = ::GetIconInfo(hIconOut, &ii);
			if (breturnue == FALSE)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
...
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	...
	if (hBitmapIn)
	{
		...
		// Get bitmap size
		nreturnue = ::GetObject(hBitmapIn, sizeof(csBitmapSize), &csBitmapSize);
		if (nreturnue == 0)
		{
			FreeResources();
			return BTNST_INVALIDRESOURCE;
		...
		if (hBitmapOut)
		{
			...
			// Get bitmap size
			nreturnue = ::GetObject(hBitmapOut, sizeof(csBitmapSize), &csBitmapSize);
			if (nreturnue == 0)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
...
DWORD CButtonST::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	...
	if (nMenu)
	{
		...
		// Load menu
		breturnue = m_menuPopup.LoadMenu(nMenu);
		// If something wrong
		if (breturnue == FALSE) return BTNST_INVALIDRESOURCE;
		...
		if (nToolbarID)
		{
			...
			breturnue = m_menuPopup.LoadToolbar(nToolbarID);
			// If something wrong
			if (breturnue == FALSE) 
			{
				m_menuPopup.DestroyMenu();
