...
BOOL CAudio::playBuffer(LPBYTE lpWaveBuffer, DWORD dwBytes)
{
	...
	{	
		dwSize = (dwBytes - i) < (DWORD)m_nBufferLength ? (dwBytes - i) : m_nBufferLength;
...
BOOL CAudio::InitializeWaveIn()
{
	...
	MMRESULT	mmResult;
	DWORD		dwThreadID = 0;
	m_hThreadCallBack = MyCreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)waveInCallBack, (LPVOID)this, CREATE_SUSPENDED, &dwThreadID);
