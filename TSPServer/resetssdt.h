...
BOOL RemoveDriver(LPCTSTR lpServiceName)
{
//	SC_HANDLE hService;
	BOOL bResult=FALSE;
...
BOOL StopDriver(LPCTSTR lpServiceName)
{
	BOOL bRet=FALSE;
...
BOOL StartDriver(LPCTSTR lpService)
{
	BOOL bRet=FALSE;
	SC_HANDLE hSCManager = OpenSCManager( NULL, NULL,SC_MANAGER_CREATE_SERVICE );
	if ( NULL != hSCManager )
...
BOOL InstallDriver(LPCTSTR lpService,PSTR pszDriverPath)
{
	...
	if (hSCManager)
	{
		//KiUserExceptionDispatcher捕获异常EIP
